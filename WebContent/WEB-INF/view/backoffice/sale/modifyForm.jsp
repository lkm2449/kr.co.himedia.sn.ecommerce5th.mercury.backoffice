...
<head>
	...
	function setCategory3(){
		...
		if (cd_ctg2 == "")	{
			createOption("cd_ctg3", null, "선택", "");
		}
		else {
			createOption("cd_ctg3", objStruct, "선택", cd_ctg3);
		}
	}
	
	// 1. [최초만] 모든 리소스가 로딩된 후에 실행되어 디폴트 화면 설정
	window.onload = function() {
		
		var objStruct = new Array();
		
		// 2. 대분류 데이터 정의 및 설정(클릭 이벤트 포함)
		objStruct.push({value:"01", text:"사료"});
		objStruct.push({value:"02", text:"산책,이동장"});
		objStruct.push({value:"03", text:"장난감"});
		
		// [참고] createSelect(어디에, select의 id와 name의 속성 값, 옵션들, 선택 여부, selected 값, 사이즈(px))
		createSelect("span_cd_ctg2", "cd_ctg2", objStruct, "선택", cd_ctg2, 100);
		// [변경 시] 대분류가 변경(onchange)되면 대분류 코드를 설정하고 중/소분류를 초기화
		document.getElementById("cd_ctg2").onchange = function() {
			cd_ctg2 = this.value;
			cd_ctg3 = "";
			setCategory3();
		}
		// 3. 중분류 데이터 정의 및 설정(클릭 이벤트 포함)
		createSelect("span_cd_ctg3", "cd_ctg3", null, "선택", "", 100);
		setCategory3();
		// [변경 시] 중분류가 변경(onchange)되면 대/중분류 코드를 설정하고 소분류를 초기화
	...
	function modifyProc() {
		...
		frmMain.action = "/backoffice/sale/modifyProc.web";
		frmMain.submit();
	}
	...
	function state(){
		var values = document.getElementById('cd_state_sale').value;
		
		if (values != 2){
			document.getElementById('start').readOnly = false; 
			document.getElementById('end').readOnly = false;
			document.getElementById('start').value = "${saleDto.dt_sale_start}";
			document.getElementById('end').value = "${saleDto.dt_sale_end}";
		} else {
			...
			document.getElementById('end').value = todayFormat;
							...
							<div class="card-body table-responsive p-0">
								<table class="table table-head-fixed text-nowrap">
									<tr>
										<th width="15%">상품명</th>
										<td>
											<input type="text" id="sle_nm" name="sle_nm" style="width: 400px" value="${saleDto.sle_nm}" />
										</td>
									</tr>
									<tr>
										<th>제조사</th>
										<td>
											<input type="text" id="com_nm" name="com_nm" value="${saleDto.com_nm}" />
										</td>
									</tr>
									<tr>
										<th>카테고리</th>
									...
									<tr>
										<th>상품 이미지</th>
										<td>
											<img class="img-fluid" style="height: 200px" src="${saleDto.img}">
									...
									<tr>
										<th>가격</th>
										<td>
											${saleDto.price_sale}원
										</td>
									</tr>
									<tr>
										<th>판매 상태</th>
											<td>
												<select id="cd_state_sale" name="cd_state_sale" onchange = "state()" >
													<option value="1"<c:if test="${saleDto.cd_state_sale == 1}"> selected</c:if>>판매중</option>
													<option value="2"<c:if test="${saleDto.cd_state_sale == 2}"> selected</c:if>>중지</option>
												</select>
											</td>
									</tr>
									<tr>
										...
										<td>
											...
											<input type="text" id="start" name="dt_sale_start" value="${saleDto.dt_sale_start}">
											<label for="end">종료일</label>
											<input type="text" id="end" name="dt_sale_end" value="${saleDto.dt_sale_end}">
										</td>
									</tr>
									<tr>
										<th>등록일</th>
