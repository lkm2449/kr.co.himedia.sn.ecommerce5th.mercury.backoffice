...
<head>
	...
	<script>		
		function goPage(value) {
			...
			frmMain.action = "/backoffice/sale/list.web";
			frmMain.submit();
		}
						...
						<div class="card">
							...
							<div class="card-body table-responsive p-0">
											...
											<c:forEach items="${list}" var="list">
												<tr>
													...
													<td style="text-align: left">
														<a href="/backoffice/sale/view.web?seq_sle=${list.seq_sle}">${list.sle_nm}</a>
													</td>
													<td>
														${list.cd_ctg}
													</td>
													<td>
														<img class="img-fluid" style="height: 100px" src="${list.img}">
													</td>
													<td>
														${list.price_sale}
													</td>
													<td>
														<c:if test="${list.cd_state_sale == 0}"> 대기</c:if>
														<c:if test="${list.cd_state_sale == 1}"> 판매중</c:if>
														<c:if test="${list.cd_state_sale == 2}"> 중지</c:if>
													</td>
													<td>
														${list.dt_reg}
													</td>
								...
								<div style="text-align:center">
									...
									<br />
									<a href="/backoffice/sale/writeForm.web" class="btnBasic"> 등록 </a>
								</div>
							</div>
							<!-- /.card-body -->
						</div>
